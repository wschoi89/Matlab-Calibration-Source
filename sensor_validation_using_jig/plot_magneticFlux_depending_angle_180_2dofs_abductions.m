...
angle_fixed=arr_angles_reference_flex(7);
...
for i=1:size(arr_angles_reference_abd, 2)
    str_folderPath = strcat('magnetic_180/2dofs/',name_sensor,'/','abd_',num2str(arr_angles_reference_abd(i)));
    
    str_fileName = strcat(str_folderPath,'/',name_sensor,'_',num2str(arr_angles_reference_abd(i)),'_',num2str(angle_fixed),'degree.csv');
...
% calculate angle
arr_angles_calculated_abd = size(arr_angles_reference_abd);% preallocate calculated angle array depending on the size of the number of reference angles 
arr_angles_calculated_flex = size(arr_angles_reference_abd);% preallocate calculated angle array depending on the size of the number of reference angles 

%flexion
for i=1:size(arr_angles_reference_abd, 2)
    arr_angles_calculated_abd(i) = atan2(arr_mean_normalized(i,1),arr_mean_normalized(i,3))*180/pi+pi/4*180/pi;
...
arr_angles_fixed_flex = zeros(1, size(arr_angles_calculated_abd,2));
arr_angles_fixed_flex(1,:) = angle_fixed;
comp_angle = [arr_angles_reference_abd' arr_angles_fixed_flex' arr_angles_calculated_flex' arr_angles_calculated_abd'];
...
legend('abduction ref','flexion ref','flexion calc', 'abduction calc');
xlabel('reference angle (degree)')
