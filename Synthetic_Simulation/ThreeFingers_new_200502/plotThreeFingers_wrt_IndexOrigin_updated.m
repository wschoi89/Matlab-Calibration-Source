...
% num_zigPos = [36, 36, 36]; % thumb, index, middle 
num_zigPos = [48, 48, 48]; % thumb, index, middle 
...
for n_pos=1:num_zigPos(finger)
    magnetic_data{1,n_pos} = zeros(num_samples, num_sensors*num_fingers); 
end
...
for n_pos=1:num_maxZigPos % the number of thumb zig positions
    ...
    fileName_magneticData=strcat('DAQ/',device_name,'/',device_name,'_DAQ_T',num2str(n_pos),'.csv');
    temp = load(fileName_magneticData);
    magnetic_data{1,n_pos}(1:num_samples,1:6) = temp(1:num_samples, 1:6); % load thumb sensors' magnet data
    
    fileName_magneticData=strcat('DAQ/',device_name,'/',device_name,'_DAQ_I',num2str(n_pos),'.csv');
    temp = load(fileName_magneticData);
    magnetic_data{1,n_pos}(1:num_samples,7:12) = temp(1:num_samples, 7:12); % load index sensors' magnet data
    
    fileName_magneticData=strcat('DAQ/',device_name,'/',device_name,'_DAQ_M',num2str(n_pos),'.csv');
    temp = load(fileName_magneticData);
    magnetic_data{1,n_pos}(1:num_samples,13:18) = temp(1:num_samples, 13:18); % load middle sensors' magnet data
