...
%          0+A4 -pi/2+B4         arr_links(3,1)+C4 0+D4;
%          0+A5 B5+off_TH3 arr_links(4,1)+C5 0+D5;
%          0 off_TH4 arr_links(5,1) 0;
%          0 -pi/2         arr_links(6,1) 0;];
%          0+A6 B6+off_TH4 arr_links(5,1)+C6 0+D6;
%          0 -pi/2         arr_links(6,1) 0;];
...
for iter=1:size(data, 1)
    ...
    error(iter, 1) = vpa(error(iter, 1), 5);
...
disp('start')
% A5;C5;D5;
parameter = [off_TH1;off_TH2;off_TH3;off_TH4;A1;B1;C1;D1;A2;C2;D2;A4;B4;C4;D4;A5;C5;D5;A6;C6;D6;];
...
options = optimoptions(@lsqnonlin,'Algorithm', 'levenberg-marquardt','Display', 'iter', 'MaxFunctionEvaluations', 1000000, 'MaxIterations', 1000000);
...
% A5;C5;D5;
output = lsqnonlin(fh,[off_TH1;off_TH2;off_TH3;off_TH4;A1;B1;C1;D1;A2;C2;D2;A4;B4;C4;D4;A5;C5;D5;A6;C6;D6;],[],[], options);    
...
% for iter=1:size(data, 1)-15
%     
%     error(iter, 1) = sqrt(sum((pos_reference(iter,:)-pos_frame7(:)').^2));
%     error(iter, 1) = subs(error(iter, 1), [B2,B3,B5,B6], [data(iter,1),data(iter,2),data(iter,3),data(iter,4)]);
%     error(iter, 1) = subs(error(iter,1));
%     
% % error = sqrt(sum((pos_frame7 - pos_reference).^2));
% % error(1,1) = sqrt(((pos_frame7(1) - pos_reference(1))^2));
% % error(2,1) = sqrt(((pos_frame7(2) - pos_reference(2))^2));
