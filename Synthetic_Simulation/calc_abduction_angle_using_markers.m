...
% data = csvread('csv_angle_ab_plus.csv');
data = csvread('csv_an gle_ab_zero.csv');
% data = csvread('csv_angle_ab_minus.csv');
% delete first row
data(1,:) = [];
...
L12_reference = 16; % length between marker 1 and 2
L34_reference = 18.9; % length between marker 2 and 3
...
% comparison = [angle_marker angle_magnet(:,1)];
comparison = [180-angle_marker angle_magnet(:,1)];
% comparison = [-180+angle_marker angle_magnet(:,1)];
% comparison = [-angle_marker angle_magnet(:,1)];
subplot(2,1,1)
plot(comparison)
