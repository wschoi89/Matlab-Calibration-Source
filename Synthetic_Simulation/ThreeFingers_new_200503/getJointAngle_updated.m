...
% 3D magnetic sensor에서 얻은 data를 이용해 joint angle 계산하는 함수
    ...
    for iter=1:length
       %  Thumb 
        ...
        jointAngle(iter,1) = - atan2(bx_p_3_thumb(iter), sqrt(by_p_3_thumb(iter)^2+bz_p_3_thumb(iter)^2))+ext_sensor_thumb_in(1);
        ...
        jointAngle(iter,4) =  atan2(bx_d_3_thumb(iter), sqrt(by_d_3_thumb(iter)^2+bz_d_3_thumb(iter)^2))+ext_sensor_thumb_in(4);

        
       % index finger
       jointAngle(iter,5) = - atan2(bx_p_3_index(iter), sqrt(by_p_3_index(iter)^2+bz_p_3_index(iter)^2))+ext_sensor_index_in(1);
       ...
       jointAngle(iter,8) =  atan2(bx_d_3_index(iter), sqrt(by_d_3_index(iter)^2+bz_d_3_index(iter)^2))+ext_sensor_index_in(4);
        
      % middle finger
      
      jointAngle(iter,9) =  - atan2(bx_p_3_middle(iter), sqrt(by_p_3_middle(iter)^2+bz_p_3_middle(iter)^2))+ext_sensor_middle_in(1);
      ...
      jointAngle(iter,12) =  atan2(bx_d_3_middle(iter), sqrt(by_d_3_middle(iter)^2+bz_d_3_middle(iter)^2))+ext_sensor_middle_in(4);
