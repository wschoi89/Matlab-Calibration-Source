...
%% training  sample
num_trainingData = 100;
num_iteration = 3;


if round(num_trainingData*3/7,-1) == 0
    num_testData = round(num_trainingData*3/7);
else
    num_testData = round(num_trainingData*3/7, -1);
end

% num_testData = num_trainingData;

num_sample = (num_trainingData + num_testData);

%% data extractaction and calculate mean by iteration number


num_DHparam_length = 10;
num_DHparam_angle = 6;
num_sensor_offset = 4;

%data consists of 14 columns(angle(4), offset angle(4), tip position(reference)(3), tip position(reflecting offset)(3))
data = zeros(num_sample*num_iteration, 14);

data_iter = zeros(num_sample,14, num_iteration);

% preallocation for optimizing parameters
list_concat_DHoffset = zeros(num_iteration, num_DHparam_length+num_DHparam_angle);

for iter=1:num_iteration
    
    % factorize num_sample into 4 combination samples 
    ...
    angle_random = -20+40*rand(1, num_sensor_offset);
    ...
    length_DHoffset = -5+10*rand(1,num_DHparam_length);

    %set offset DH parameter angles between -20 and 20 (degree)
    angle_DHoffset = -20+40*rand(1,num_DHparam_angle);
    angle_DHoffset = angle_DHoffset*pi/180;

    %optional for calibration
    angle_offset = zeros(1, num_sensor_offset);
    ...
    concat_DHoffset = cat(2, length_DHoffset, angle_DHoffset);
    list_concat_DHoffset(iter, :) = concat_DHoffset;
    ...
    %                     xlim([-50 200]);ylim([-200 50]);zlim([-100 100]);
                    ...
                    data(count+num_sample*(iter-1), 9:11) = pos_frame7;
...
%                     title({'X-Y', strcat('angle: ', num2str(angle_TH1), ', ', num2str(angle_TH2), ', ', num2str(angle_TH3), ', ', num2str(angle_TH4))});
    ...
    %                 xlim([-50 200]);ylim([-200 50]);zlim([-100 100]);
                ...
                data(count+num_sample*(iter-1), 1:4) = angle_device;
                data(count+num_sample*(iter-1), 5:8) = angle_offset;
                data(count+num_sample*(iter-1), 12:14) = pos_frame7_offset;
    ...
    data_iter(:,:,iter) = data(1+num_sample*(iter-1):num_sample*(iter),:);
end
...
save(strcat('offset_', num2str(num_trainingData),'.mat'), 'list_concat_DHoffset');
disp(strcat('offset data was saved in offset_', num2str(num_trainingData),'.mat file'));

% shuffle data in rows and save
% shuffle data and divide into training(70) and test data(30)

[row, col, page] = size(data_iter);

data_iter = data_iter(randperm(row),:,:);

trainingData = data_iter(1:num_trainingData,:,:);
testData = data_iter(num_trainingData+1:end,:,:);
save(strcat('trainingData_', num2str(num_trainingData), '.mat'), 'trainingData');
save(strcat('testData_', num2str(num_trainingData), '.mat'), 'testData');

disp(strcat('Training data was saved in trainingData_', num2str(num_trainingData), '.mat file'));
disp(strcat('Test data was saved in testData_', num2str(num_trainingData), '.mat file'));
% disp('finish')
% close all

% for autonomous calibration
nonSymbolic_onlyDH_allCalibration
validate_testData_onlyDH_allCalibration
