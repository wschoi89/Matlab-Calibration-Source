...
syms off_TH1 off_TH2 off_TH3

% to do: off_TH1, off_TH2 test 완료 후 모두 symbolic 변수로 바꾸기
DHRef = [0 0         0             -pi/2;
         0 0+off_TH1 arr_links(1,1) pi/2;
         0 0+off_TH2 arr_links(2,1) 0;
         0 -pi/2     arr_links(3,1) 0;
         0 0+off_TH3 arr_links(4,1) 0;
         0 0         arr_links(5,1) 0;
...
pos_reference = [-12.810;-58.928;7.396;];
% pos_reference = [43.24;-87.7;0];
...
disp('start')
parameter = [off_TH1;off_TH2;off_TH3];
...
options = optimoptions(@lsqnonlin,'Algorithm','trust-region-reflective', 'Display', 'iter', 'MaxFunctionEvaluations', 500000, 'MaxIterations', 40000);
output = lsqnonlin(fh,[off_TH1;off_TH2;off_TH3;], [], [], options);    
