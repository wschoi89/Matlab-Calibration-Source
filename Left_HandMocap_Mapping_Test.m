...
% FingerOrigin_MI=eye(4)*transl(Joint7_MI(1)-117.581,Joint7_MI(2),-Joint7_MI(3))*troty(pi/2);
FingerOrigin_MI=eye(4)*transl(0,Joint7_MI(2),Joint7_MI(3))*troty(pi/2);

% 180308 추가
% FingerOrigin_MI(1:3,4) = 0.9 * FingerOrigin_MI(1:3,4);

% Fingeroffset_MI=acos(R7_MI(1,2))-pi/2;
Fingeroffset_MI=acos(R7_MI(2,1))-pi/2;
% R7_MI=R7_MI*trotz(Fingeroffset_MI);
% Fingeroffset_MI = 0;
% FingerLength_MI = 117.581*0.95;
FingerLength_MI = Joint7_MI(1)*0.9;
...
% 180308 추가
% FingerOrigin(1:3,4) = 0.9 * FingerOrigin(1:3,4);

% FingerOrigin=FingerOrigin_MI*transl(31.792*scale2,-4.864*scale2,-0.374*scale2);
...
% 180308 추가
% FingerOrigin_TH(1:3,4) = 0.9 * FingerOrigin_TH(1:3,4);
...
% newaxis=[newaxis'*FingerOrigin_TH(1,1:3)' newaxis'*FingerOrigin_TH(2,1:3)' newaxis'*FingerOrigin_TH(3,1:3)']'
 newaxis = FingerOrigin_TH(1:3, 1:3) * newaxis;

ratio_TH = 0.8;

HandOrigin=FingerOrigin(1:3,4)+[-105.484 1.282 -37.7]'*scale3 * ratio_TH;
...
FingerOrigin=FingerOrigin_MI*transl(-31.792*scale3*ratio_index,-4.864*scale3*ratio_index,-0.374*scale3*ratio_index);
...
% TH1_F_P3(iter)=atan2(P3_af(1), P3_af(3));
...
HandOrigin(3) = HandOrigin(3)+Joint7_MI(3)-FingerOrigin_MI(3,4);
...
% FingerOrigin_MI(3,4)=Origin_MI(3,4);
